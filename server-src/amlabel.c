    ...
    char **	argv)
{
    ...
    if (!start_ndmp_proxy(&errmsg)) {
	fprintf(stderr,"Can't start ndmp-proxy: %s\n", errmsg);
	exit(1);
    }
    ...
    if (device->status != DEVICE_STATUS_SUCCESS) {
        fprintf(stderr,_("Could not open device '%s': %s.\n"), tapename,
	        device_error(device));
	dbclose();
	exit(1);
    }
    ...
    if (!device_configure(device, TRUE)) {
        fprintf(stderr,_("Could not configure device '%s': %s.\n"), tapename,
	        device_error(device));
	dbclose();
	exit(1);
    }
