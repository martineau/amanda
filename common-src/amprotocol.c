...
#include <errno.h>
#include <arpa/inet.h>
#include "amprotocol.h"

static ssize_t my_read(int fd, void *buf, ssize_t count);
static ssize_t my_write(int fd, const void *buf, ssize_t count);
    ...
    amprotocol_packet_t *packet)
{
    ...
    return my_write(protocol->fd, buffer, block_size);
}
    ...
    amprotocol_t *protocol)
{
    ...
    size = my_read(protocol->fd, &command, sizeof(command_t));
    save_errno = errno;
	...
	size = my_read(protocol->fd, buffer, packet->block_size - sizeof(command_t));
